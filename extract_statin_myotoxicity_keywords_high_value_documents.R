...
##  Input: directory (the directory containing only high value document txt files), wordspan (the number of words to either side of the statin, in high value documents we found a wordspan of 8 - thus total phrase length of 16 to be acceptable), threads (the number of threads for snowfall to distribute the task over, 1 defaults to a single thread process), method (either corrected or original related to which set of keywords to use for tagging.)
